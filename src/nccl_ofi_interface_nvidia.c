...
static ncclResult_t getProperties_v8(int dev_id, ncclNetProperties_v8_t* props)
{
	...
	if (ofi_properties.mr_scope == NCCL_OFI_MR_SCOPE_DOMAIN) {
		 ...
		 *
		 * Even though aws-ofi-nccl registers MRs globally (As MRs
		 * registered on EFA are not specific to a communicator),
		 * aws-ofi-nccl doesn't have such a fast MR cache yet.
		 * Therefore, it should return regIsGlobal=0 for now. We should
		 * re-enable this when we fix the perf problem.
		 */
		props->regIsGlobal = 0;
	}
